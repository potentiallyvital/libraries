...
function countdown($date, $callback = null)
{
	if (!is_numeric($date))
        {
                $date = strtotime($date);
	...
	else
	{
		...
		return '<span class="countdown" data-time="'.$date.'" data-callback="'.$callback.'">'.$time.'</span>';
	}
}
...
function getTimeUntil($date, $format = null)
{
        ...
        if ($seconds <= 1){
                ...
                return "in <span class=\"time-until\">$seconds</span> seconds";
        } elseif ($minutes == 1){
